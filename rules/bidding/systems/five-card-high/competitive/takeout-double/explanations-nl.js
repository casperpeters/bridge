  ...
  function explainTakeoutDoubleChoice(ruleName, result, helpers = {}) {
    ...
    switch (ruleName) {
      case "competitive.takeoutDouble":
        return `informatiedoublet: 12+ punten, kort in ${suitName(result.opponentSuit)}, en steun voor de ongeboden kleuren; met 16+ punten mag een ongeboden kleur soms een driekaart zijn. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      ...
      case "competitive.takeoutDoubleOneNotrump":
        return `1SA na partners informatiedoublet: 6-9 punten, SA-verdeling en dekking in ${suitName(result.stopperSuit || result.opponentSuit)}; dit gebeurt alleen zonder betere biedbare vierkaart. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleJumpSuit":
        return `sprongantwoord na partners informatiedoublet: met 9-11 punten spring je in de langste nog niet door de tegenpartij geboden kleur; bij gelijke lengte kies je de hoogste. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleGame":
        return `manche na partners informatiedoublet: vanaf 12 punten ga je uit van opening tegenover opening en zorg je dat de partij de manche bereikt. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleVoluntarySuit":
        return `vrijwillig bod na partners informatiedoublet: de tegenpartij heeft na het doublet geboden, dus de biedplicht vervalt; met voldoende punten bied je een nog niet geboden vierkaart. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleRebidInviteRaise":
        return `invite na partners antwoord op jouw informatiedoublet: met 17-19 punten en fit steun je partners kleur met een sprong. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleRebidInviteNotrump":
        return `2SA na partners antwoord op jouw informatiedoublet: 17-19 punten, gebalanceerde hand en dekking in ${suitName(result.stopperSuit || result.opponentSuit)}. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleRebidGameRaise":
        return `manche na partners antwoord op jouw informatiedoublet: met 20+ punten en fit verhoog je naar de kleurmanche. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
      case "competitive.takeoutDoubleRebidGameNotrump":
        return `3SA na partners antwoord op jouw informatiedoublet: 20+ punten, gebalanceerde hand en dekking in ${suitName(result.stopperSuit || result.opponentSuit)}. ${handFactsText({ ruleName, result, valueMode: "hcp" })}`;
  ...
  function explainPassChoice(ruleName, result, helpers = {}) {
    ...
    switch (ruleName) {
      case "competitive.takeoutDoubleRebidPassMinimum":
        return `herbieding na partners antwoord op jouw informatiedoublet: met 12-16 punten accepteer je partners antwoord en pas je${factSuffix}`;
