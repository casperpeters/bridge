  ...
  function explainOpeningBidChoice(ruleName, result, helpers = {}) {
    ...
    switch (ruleName) {
      case "opening.oneNotrump":
        return `Vijfkaart Hoog: open 1SA met 15-17 punten en een gebalanceerde hand. ${handFactsText({ ruleName, result })}`;
      case "opening.twoNotrump":
        return `Vijfkaart Hoog: open 2SA met 20-22 punten en een gebalanceerde hand. ${handFactsText({ ruleName, result })}`;
      ...
      case "opening.weakTwo":
        ...
        return `zwakke twee in ${suitName(result.suit)}: 6-10 punten, of een lelijke 11-punter die de Regel van 20 niet haalt, met een goede exacte 6-kaart. ${handFactsText({ ruleName, result })}`;
      case "opening.preempt":
        ...
        return `preemptieve opening in ${suitName(result.suit)}: 7-10 punten met een goede ${result.length >= 8 ? "8+-kaart op vierniveau" : "7+-kaart op drieniveau"} en minstens twee honneurs in de kleur. ${handFactsText({ ruleName, result })}`;
